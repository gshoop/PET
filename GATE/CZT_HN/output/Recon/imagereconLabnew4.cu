...
int msize;
float torhw, torsgm;
float betas, sumpop, sumimp, sumb;
...
int main(int argc, char* argv[])
{
	...
	if (config.is_open())
	{
		while ( getline (config,line) )
		{
			...
			if(vpara[0]=="BetaR") {ss<<vpara[2];ss>>betas;ss.clear();} else
	...
	if(rgl==1)
	{
		cout<<"Beta for regularization: "<<betas<<endl;
		cout<<"Blur?: "<<blur<<endl;
        if(blur==1)
        {
...
//re-define beta, beta_new = beta * A / 2. Also initialize smatrix.
	if(norma == 0) {
		betas = betas * float(numline)/msize / 2.0f;
		...
		betas = betas * float(numline)/sumNormimage / 2.0f;
